...
def index():
    ...
    return render_template("/HTML/index.html", data=session['books'], message=message, username=username)
...
def bookpage(isbn):
    ...
    data = db.execute("SELECT * FROM books WHERE isbn = :isbn",
                      {"isbn": isbn}).fetchone()
    return render_template("/HTML/book.html", data=data, reviews=session['reviews'], average_rating=average_rating, work_ratings_count=work_ratings_count, username=username, warning=warning)
        ...
        if emailLogIn == None:  # registration
            ...
            for i in range(len(data)):
                if data[i]["username"] == email:
                    ...
                    return render_template('/HTML/login.html', log_in_message=log_in_message)
